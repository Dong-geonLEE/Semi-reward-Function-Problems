...
class RLTransformer(nn.Module):
    def __init__(self, d_embed=50, n_head=10,
        ...
        self.env = CustomEmptyEnv(size=env_size, max_steps=self.max_step)
        # self.env = CustomLavaGapEnv(size=env_size, max_steps=self.max_step)
        # self.env = CustomDistShiftEnv(width=env_size+2, height=env_size, strip2_row=2, max_steps=self.max_step)
    ...
    def episode(self, mode='exploitation', pre_epi_steps=1):
        ...
        self.env.agent_pos = (rd.choice(range(1, self.env_size-2)),
                              rd.choice(range(1, self.env_size-2)))
        self.env.agent_pos = (rd.choice(range(1, 3)), rd.choice(range(1, 3)))
        self.env.agent_dir = rd.choice(range(4))
    ...
    def predict(self, observation, mode='evaluation'):
    ...
    def train(self, optimizer, scheduler, tot_steps):
        ...
        while steps < tot_steps:
            mode = 'exploitation'
            episode_buffer, epi_obs_buffer, actions, epi_reward, epi_step = self.episode(mode=mode, pre_epi_steps=pre_epi_step)
            ...
            if epi % 100 == 0:
                ...
                self.tot_rew.append(np.sum(rewards[-100:]) / 100)

                if round((success / 100) * 100, 2) > 50:
                    print('early evaluation')
                    self.eval()
                    torch.save(self.embedding_layer, './custom/trained_model/empty_embedding.pt')
                    torch.save(self.decoder, './custom/trained_model/empty_rl_trans_decoder.pt')
                    torch.save(self.encoder, './custom/trained_model/empty_rl_trans_encoder.pt')
    ...
    def eval(self):
        ...
        for size, step in size_step:
            ...
            for i in range(10):
                ...
                while not done:
                    ...
                    action = self.predict(obs, mode='evaluation')
        ...
        for step, row in step_row:
            ...
            print(f'dist shift size 2row {row} step {step}')
            print(rewards)
            print(np.mean(rewards))
            print(np.std(rewards))
