...
#Semi-Reward Function Environment
...
class CustomMiniGridEnv(gym.Env):
    ...
    def _reward(self) -> float:
        ...
        return round(-1 + 0.9 * (self.step_count / self.max_steps), 4)
        # return -1

    def _goal_reward(self) -> float:
        """
        Compute the classical reward for goal arrival
        -> 구현의 편의를 위해, 환경에 적용되어 있음.
         실제로는 agent가 현재 상황 = goal인경우 판단해서 자체적으로 reward를 부여해가도록 하여도
         큰 차이가 없음.
        """
        return round(1 - 0.9 * (self.step_count / self.max_steps), 4)
        # return 1
    ...
    def step(
        ...
        # Pick up an object
            ...
            if fwd_cell and fwd_cell.can_pickup():
                ...
                # custom: do pick up while carrying one
                # else:
                #     terminated = True
                #     reward = self._reward()

            # custom: fwd cannot pick up
            elif fwd_cell and fwd_cell.can_pickup() is False:
                terminated = True
                reward = self._reward()
        ...
        # Drop an object
            ...
            # drop while carrying noting
            elif not fwd_cell and not self.carrying:
                terminated = True
                reward = self._reward()
